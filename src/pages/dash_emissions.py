...
df = pd.read_parquet(
    "~/positive-disruption-dev/src/data/emissions_output_co2e.parquet"
).reset_index()
    ...
    [
        html.Div(
            children=[
                ...
                html.Br(),
                html.Label(
                    "% Land Available for Agroforestry",
...
def update_graph(
    ...
    df = pd.read_parquet("~/positive-disruption-dev/src/data/" + dataset + ".parquet")
    ...
    # multiply Agroforestry by range slider value
    df[(df.reset_index().product_long == "Agroforestry").values] = df.loc[
        (df.reset_index().product_long == "Agroforestry").values
    ].multiply(
        np.full_like(
            df.loc[(df.reset_index().product_long == "Agroforestry").values],
            af_slider[0] / 100,
        ),
        axis=0,
    )
